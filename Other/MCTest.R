MonteCarloPrice <- function(mSteps, NSteps, nSim, GF, LF, LC, Volatility, InterestRate){
    ...
    Payoff = 0
    S = 0
    Stock <- matrix(nrow=nSim, ncol= mSteps*N_Obs)

    for(j in 1:nSim){                                           # For all simulated paths.
        for(i in 1:(mSteps*NSteps)){                            # For all time steps.
            ifelse(i==1, S <- S0, S <- Stock[j,i-1])            # The stock price is updated for each time step on the simulated paths.
            Stock[j,i] <- S*exp((r-((Volatility)^2)/2)*dt+Volatility*sqrt(dt)*rnorm(1,0,1))}                      
        ...
        for(i in 1:NSteps){                                     # For all observation dates (i.e. the dates at which payoff is to be calculated).
            ifelse(i==1, S1 <- S0, S1 <- Stock[j,(i-1)*mSteps])  
            S2 <- Stock[j,i*mSteps]
...
MonteCarloPrice(300, 5, 1000, 0.16, 0, 0.08, 0.25, 0.03)
