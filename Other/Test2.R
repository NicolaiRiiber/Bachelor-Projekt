...
library(lubridate) # This function requires the package 'lubridate' which transforms seconds into the format HH:MM:SS.

Cliquet <- function(mSteps, QSteps, NSteps, GlobalFloor, LocalCap, Volatility, IntRate){
  
  CompTime = proc.time() # Starting timer.
  CliquetPrice = vector(length = length(Volatility)) # Empty vector that will be filled with option prices corresponding to each volatility.
  LocalFloor = 0
  r = log(1 + IntRate)
      ...
      for (j in 1:QSteps){                 # For all Q-steps (i.e. all possible payoffs).
        for (t in mSteps:1){               # For all m-steps (i.e. all sub-periods).
          for (i in 1:t){                  # For all knots.
            ifelse((t == mSteps), c(Interpolate <- approxfun(Q,ValueObs,method="linear",rule=2), Q_up <- xi[i]*u, Q_down <- xi[i]*d, Values[i,t]<-exp(-r*dt)*(p*Interpolate(Q[j]+min(LocalCap,max(LocalFloor, Q_up-1)))+(1-p)*Interpolate(Q[j]+min(LocalCap,max(LocalFloor,Q_down-1))))), (Values[i,t] <- exp(-r*dt)*(p*Values[i+1, t+1]+(1-p)*Values[i,t+1])))
          }
        }
  ...
  if (as.numeric(proc.time() - CompTime)[3] >= 60){
    CompTimeEnd = round(seconds_to_period(as.numeric(proc.time() - CompTime)[3]), 0)           # Calculates the elapsed computation time.
    print(paste("The elapsed computation time was:", CompTimeEnd))}                            # Prints the elapsed computation time.
  else{
    CompTimeEnd = paste(round(as.numeric(proc.time() - CompTime)[3], 0), "seconds")            # Calculates the elapsed computation time.
    print(paste("The elapsed computation time was:", CompTimeEnd))}                            # Prints the elapsed computation time.
  
  if(length(CliquetPrice) == 1){                                                  # Prints contract value when volatility is constant.
    print(paste("The contract value is:", round(CliquetPrice, 5)))}               # Prints contracts calue when volatility is fixed.
  else{
...
vol <- c(seq(0.20, 0.25, length = 10))
Cliquet(10, 10, 5, 0.16, 0.08, 0.20, 0.03)
